...
# Function to extract the keygrip for the subkey with [A] capability
get_keygrip() {
    #  `awk -F: '/^sub/{f=0} f{print $10} /^sub.*:a:/{f=1}'`:
    #  This command extracts the keygrip of the subkey with the [A] capability. It works as follows:
    #    `/^sub/{f=0}`: When the line starts with sub, set f to 0 (false).
    #    `f{print $10}`: If f is true, print the 10th field (keygrip).
    #    `/^sub.*:a:/{f=1}`: If the line matches sub.*:a:, set f to 1 (true).
    # `sed '2q;d'`: print only the second line
    gpg -k --with-colons --with-keygrip "$1" \
        | awk -F: '/^sub/{f=0} f{print $10} /^sub.*:a:/{f=1}' \
        | sed '2q;d'
}

# Command to update sshcontrol file with the keygrip for the subkey with [A] capability
sshcontrol() {
    subkey_line=$(gpg -k --with-colons --with-keygrip "$SIGNER" | awk -F: '/^sub.*:a:/{print}')
    if [ -n "$subkey_line" ]; then
        keygrip=$(get_keygrip "$SIGNER")
        if [ -n "$keygrip" ]; then
            # The $ in $s/.*/"$keygrip"/ selects the last line of the file, and the .* matches the entire line,
            # which is replaced with the contents of the KEYGRIP variable.
            sed -i.bak '$s/.*/'"$keygrip"'/' ~/.gnupg/sshcontrol
            echo "Updated sshcontrol file with keygrip: $keygrip"
        else
            echo "Error: Failed to extract keygrip for subkey with [A] capability"
        fi
    else
        echo "Error: No subkey found with [A] capability"
    fi
}
...
case "$1" in
	...
	usage
        ...
        echo "  sshcontrol                          Update the SSH control file (~/.gnupg/sshcontrol) with the keygrip of the signer key "
